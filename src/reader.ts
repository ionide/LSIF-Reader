...
export class LsifReader {
    ...
    public load(content: string, transformerFactory: (workspaceRoot: string) => UriTransformer) {
        ...
        if (this.version === undefined) {
            ...
            //     throw new Error(`Requires version range >0.5.99 <=0.6.0-next.4 but received: ${this.version}`)
            // }
        }
    ...
    private doProcessDocument(document: Document): void {
        ...
        if (hash !== value.hash) {
            throw new Error(`Document ${document.uri} has different content.`)
        }
    ...
    private toDocumentSymbol(value: RangeBasedDocumentSymbol): lsp.DocumentSymbol | undefined {
        ...
        let result: lsp.DocumentSymbol = lsp.DocumentSymbol.create(tag.text, tag.detail || "", tag.kind, tag.fullRange, this.asRange(range))
    ...
    private findRangesFromPosition(file: string, position: lsp.Position): Range[] | undefined {
        ...
        for (const document of value.documents) {
            ...
            for (let item of contains) {
                ...
                if (LsifReader.containsPosition(item, position)) {
                    if (!candidate) {
                        candidate = item
                    } else {
                        if (LsifReader.containsRange(candidate, item)) {
                            candidate = item
                        }
                    }
                }
